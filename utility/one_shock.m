...
% Last line below isn't working for some reason...
for jj = 1 : numshk
    eval(['targetnum' num2str(jj) '= strmatch(targetvar' num2str(jj) ',M_.endo_names,''exact'');']);
    eval(['numexo = size(exovars' num2str(jj) ',1);']);
    for kk = 1 : numexo
    eval(['exonum' num2str(jj)    '(' num2str(kk) ',:) = strmatch(exovars' num2str(jj) '(' num2str(kk) ',:),M_.exo_names,''exact'');']);
    end
end
