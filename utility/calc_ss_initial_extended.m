...
%
% =========================================================================
function out = calc_ss_initial_extended(SET)
...
x0      = log(SET.x0);
x0      = [x0; log(param.lva_weights./(1-param.lva_weights))];
x0      = [x0; log(param.va_weights./(1 - param.va_weights))];
x0      = [x0; zeros(nsectors-1,1)];
...
% ========================================================================
    function F = ssfunc(x0)
        ...
        w = zeros(nsectors,1);
        x = zeros(nsectors,1);
        omega_nd = zeros(nsectors,1);
        omega_y  = zeros(nsectors,1);
        omega_ns = zeros(nsectors,1);
        ...
        for m_ = 1 : nsectors
            omega_nd_(m_,1)= x0(j_)./(1 + x0(j_)); j_ = j_ + 1;
        end

        for m_ = 1 : nsectors
            omega_y_(m_,1) = x0(j_)./(1 + x0(j_)); j_ = j_ + 1;
        end

        for m_ = 2 : nsectors
            omega_ns_(m_,1) = x0(j_); j_ = j_ + 1;
        end

        omega_ns_(1) = 5;  % Normalisation
        %omega_ns_(1) = 1;  % Normalisation
        %omega_ns_(1) = 1 - sum(omega_ns_(2:end));
        ...
        % Aggregate labour
        lab =  (sum(omega_ns_.^(-1/param.xi).*n_.^((param.xi+1)/param.xi))) ^(param.xi/(param.xi+1));
        ...
        F = [
            ...
            omega_ns_.^(-1/param.xi) .* A_N .* lab.^(param.nu - 1 / param.xi) ...
            ...
            % Captital share of income must match data
            rk .* k_ ./(f_.* gam_f_) - (1-param.lva_weights);

            % Value-add share of gross output must match data
            f_ .* gam_f_ ./ (f_ .*gam_f_ + x_ .* gam_x_) - param.va_weights;

            % Hrs worked share in each industry must match data
            n_(2:end) ./ sum(n_) - param.employment_share(2:end);
